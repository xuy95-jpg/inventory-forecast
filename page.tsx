...
import { Save, Settings, Plus, X, Trash2 } from 'lucide-react';
...
export default function DataEntryPage() {
  ...
  // When date changes, pre-fill from existing data
  useEffect(() => {
    ...
    setRows(prev => {
      ...
      Object.keys(next).forEach(k => {
        next[k] = { sales: '', cut: '', whole: '', wastage: '', soldOut: false };
      });
      // Then fill in from existing records
      existing.forEach(r => {
        next[r.skuId] = {
  ...
  const handleDelete = () => {
    const recordsToDelete = Object.entries(rows)
      .filter(([, v]) => v.sales !== '' || v.cut !== '' || v.whole !== '' || v.wastage !== '');
    if (recordsToDelete.length === 0) return;
    if (!confirm(`确认删除 ${date} 的 ${recordsToDelete.length} 条已录入数据？此操作不可恢复。`)) return;
    // Delete by clearing values and saving empty records
    addSalesRecords(recordsToDelete.map(([skuId]) => ({
      date, storeId, skuId,
      salesQuantity: 0, cutStock: 0, wholeStock: 0, wastage: 0, soldOut: false,
    })));
    setRows(prev => {
      const next = { ...prev };
      recordsToDelete.forEach(([skuId]) => {
        next[skuId] = { sales: '', cut: '', whole: '', wastage: '', soldOut: false };
      });
      return next;
    });
    setSaved(-1); // negative means deleted
    setTimeout(() => setSaved(0), 2000);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {filledCount > 0
              ? `📝 ${date} 已录入 ${filledCount} 个品 · 可直接修改`
              : `填入数值即可 · ${filledCount} 个品已填`}
          </p>
        </div>
